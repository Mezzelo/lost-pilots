-- Slightly modified version of replaceRepair for use in Lost Pilots.  Get a clean copy!
...
local function GetVisiblePawn()
    if not GetCurrentMission() and Board then
        for i = 0,2 do
            local pawn = Board:GetPawn(i)
            if pawn then return pawn end
        end
        return nil
    elseif Selected and Board:GetPawn(Selected) then
		return Board:GetPawn(Selected)
...
local function GetRepairSkill()
	local pawn = InTipImage() and Board:GetPawn(Selected) or Pawn
	...
	for _, v in ipairs(replaceRepair.weapons) do
		if pawn:IsAbility(v.sPilotSkill) or v.sMech == pawn:GetType() then
			-- if pawn:IsAbility("RepSnipe") and GAME.MezzPData[9][2] or (not pawn:IsAbility("RepSnipe")) then
				return v.sSkill
			-- end
		end
	end
...
local function SetupBoard(board, sSkill)
	...
	if unitPoint then
		local unitType = tipImage.CustomPawn or Board:GetPawn(Selected):GetType()
...
function replaceRepair:init(mod, globalModApiExt)
	modApiExt = globalModApiExt
...
function replaceRepair:load(mod, options, version)
	modApiExt:load(mod, options, version)
